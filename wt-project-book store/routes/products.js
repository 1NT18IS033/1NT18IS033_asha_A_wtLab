...
router.get("/:slug", async (req, res) => {
  ...
  try {
    ...
    const allProducts = await BOOK.find({ category: foundCategory.id })
      .sort("-createdAt")
      .skip(perPage * page - perPage)
      .limit(perPage)
      .populate("category");

    const count = await BOOK.count({ category: foundCategory.id });
...
router.get("/:slug/:id", async (req, res) => {
  ...
  const errorMsg = req.flash("error")[0];
  try {
    const product = await BOOK.findById(req.params.id).populate("category");
